<div class="page-body">
	<div class="overview">
		<h1>Travel &amp; Hotel</h1>
		<p class="constrained">
			Our wedding, both ceremony and reception, will be held at Butternut Farm Golf Club in Stow,
			Massachusetts. About 25 minutes northeast of Worcester and 45 minutes west of Boston, Stow is
			a small town known for apple picking and beautiful fall foliage.
		</p>
	</div>

	<div class="gradient-border">
		<div class="card hotel">
			<h2>Hotel</h2>
			<p>
				We've organized a hotel block at the Embassy Suites by Hilton Boston Marlborough. Below is
				the booking information so you can get a room at a discounted rate, because everyone loves a
				bargain.
			</p>
			<p>
				Call the hotel at 508-485-5900 and select the option “0” to speak with a front desk agent
				and ask to book a room under the LeKania Wedding Or Follow the below link to our personal <a
					href="https://www.hilton.com/en/attend-my-event/bosmaes-906-a1b8c773-d681-41f4-93ce-4bac4637ae23/"
					>booking website</a
				>!
			</p>
			<p>
				Please don't hesitate to text or email us with any questions or if you run into any issues
				while booking!
			</p>
		</div>
	</div>

	<div class="gradient-border">
		<div class="card transportation">
			<h2>Transportation</h2>
			<p>
				For convenience (and safety!) of our guests, we have organized transportation for guests
				staying at the hotel to and from the venue! Currently, we are working on finalizing the
				scheduling details, but you can plan on having transportation from the hotel to our venue
				before the ceremony and back to the hotel at the conclusion of the night.
			</p>
			<p>
				Final details, including transportation departure time, will be posted to this page when it
				is available.
			</p>
		</div>
	</div>
</div>

<style>
	.page-body {
		display: grid;
		grid-template-columns: 1fr;
		gap: 4rem 2rem;
		padding: 2rem var(--padding-h);
		/* text-align: left; */
	}

	.gradient-border {
		display: flex;
		padding: 4px;
		border-radius: 12px;
		background: linear-gradient(to top left, white, var(--pink), white);
	}

	.card {
		border-radius: 8px;
		padding: 1.5rem 1rem;
		background: white;

		& h2 {
			margin-top: 0;
		}
	}

	@media (min-width: 800px) {
		.overview {
			grid-column: span 2;
		}

		.page-body {
			grid-template-columns: 1fr 1fr;
		}

		.card {
			padding: 1.5rem 2rem;
		}
	}
</style>
